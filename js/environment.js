...
// ── TYPE 5: PALM TREE (curved trunk, fan leaves) ──
function createPalmTree(x, y, z, scale) {
    const g = new THREE.Group();
    ...
    for (let si = 0; si < 6; si++) {
        ...
        const seg = new THREE.Mesh(new THREE.CylinderGeometry(0.1 * scale, 0.14 * scale, segH, 6), trunkMat);
        const curveX = Math.sin(si * 0.15) * 0.3 * scale;
        seg.position.set(curveX, si * segH + segH / 2, 0);
        seg.rotation.z = si * 0.04;
        seg.castShadow = true; g.add(seg);
    }
    // Palm fronds (leaf shapes from boxes)
    const leafMat = new THREE.MeshStandardMaterial({ color: 0x228B22, roughness: 0.8 });
    const leafMat2 = new THREE.MeshStandardMaterial({ color: 0x2d9e2d, roughness: 0.8 });
    const topY = 4.5 * scale;
    const topX = Math.sin(5 * 0.15) * 0.3 * scale;
    for (let li = 0; li < 7; li++) {
        const frondAngle = (li / 7) * Math.PI * 2;
        const frond = new THREE.Mesh(
            new THREE.BoxGeometry(0.2 * scale, 0.04 * scale, 2.2 * scale),
            li % 2 === 0 ? leafMat : leafMat2
        );
        frond.position.set(
            topX + Math.cos(frondAngle) * 0.3 * scale,
            topY - 0.2 * scale,
            Math.sin(frondAngle) * 0.3 * scale
        );
    ...
    // Coconut cluster
    const coconutMat = new THREE.MeshStandardMaterial({ color: 0x8B4513, roughness: 0.7 });
    for (let ci = 0; ci < 3; ci++) {
        const coco = new THREE.Mesh(new THREE.SphereGeometry(0.12 * scale, 6, 6), coconutMat);
        coco.position.set(topX + (Math.random() - 0.5) * 0.2 * scale, topY - 0.4 * scale, (Math.random() - 0.5) * 0.2 * scale);
        g.add(coco);
    }
    g.position.set(x, y, z); scene.add(g); return g;
}
